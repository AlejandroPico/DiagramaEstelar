...
function bind() {
  ...
  viewport.onwheel = e => {
    e.preventDefault();
    zoomAt(e.clientX, e.clientY, Math.exp(-e.deltaY * .0012));
  };
...
function updateChartMetrics() {
  ...
  const compact = w < 720;
  const left = compact ? 70 : 104;
  const right = compact ? 16 : 22;
  const top = compact ? 58 : 46;
  const bottom = compact ? 78 : 88;
  chart.width = w;
  chart.height = h;
  chart.plot = {
    x: left,
    y: top,
    w: Math.max(180, w - left - right),
    h: Math.max(180, h - top - bottom)
  };
}
...
function zoomAt(cx, cy, factor) {
  ...
  state.scale = clamp(state.scale * factor, .65, 8);
...
function panel() {
  ...
  const p = chart.plot;
  const glow = ctx.createRadialGradient(p.x + p.w * .12, p.y + p.h * .1, 0, p.x + p.w * .12, p.y + p.h * .1, Math.max(p.w, p.h));
  glow.addColorStop(0, document.body.classList.contains('dark') ? 'rgba(116,154,255,.18)' : 'rgba(116,154,255,.25)');
  glow.addColorStop(.45, 'rgba(255,224,135,.11)');
  glow.addColorStop(1, 'rgba(255,112,78,.10)');
  ctx.fillStyle = glow;
  ctx.fillRect(0, 0, chart.width, chart.height);
}
...
function bands() {
  ...
  for (const [lab, min, max, col] of spectralBands) {
    ...
    ctx.fillStyle = alpha(css('--ink'), .55);
    ctx.font = `900 ${Math.max(18, Math.min(28, chart.width / 46))}px system-ui`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(lab, (x1 + x2) / 2, p.y + p.h + 34);
  }
}

function grid() {
  const p = chart.plot;
  ctx.strokeStyle = alpha(css('--ink'), .12);
  ctx.fillStyle = alpha(css('--ink'), .66);
  ctx.lineWidth = 1 / state.scale;
  ctx.font = `800 ${Math.max(11, Math.min(18, chart.width / 78))}px system-ui`;
  ctx.textBaseline = 'middle';
  [40000,30000,20000,10000,7500,6000,5200,3700,3000,2500].forEach(t => {
    const x = tempX(t);
    line(x, p.y, x, p.y + p.h);
    ctx.textAlign = 'center';
    ctx.fillText(`${Math.round(t / 1000)}k`, x, p.y + p.h + 18);
  });
  [1e6,1e5,1e4,1e3,100,10,1,.1,.01,.001,.0001].forEach(l => {
    const y = lumY(l);
    line(p.x, y, p.x + p.w, y);
    ctx.textAlign = 'right';
    ctx.fillText(l >= 1000 ? `10^${Math.round(Math.log10(l))}` : String(l).replace('0.', '.'), p.x - 12, y);
  });
  ctx.fillStyle = css('--ink');
  ctx.font = `900 ${Math.max(13, Math.min(22, chart.width / 72))}px system-ui`;
  ctx.textAlign = 'center';
  ctx.fillText('Temperatura efectiva superficial, K  ⟵ más fría', p.x + p.w / 2, chart.height - 24);
  ctx.save();
  ctx.translate(chart.width < 720 ? 23 : 35, p.y + p.h / 2);
...
function density() {
  ...
  state.synthetic.forEach(s => {
    const x = tempX(s.teff), y = lumY(s.luminosity), r = 16 / state.scale;
...
function drawStar(s) {
  const x = tempX(s.teff), y = lumY(s.luminosity);
...
function labels() {
  ...
  items.forEach(([txt, t, l, a, c]) => {
    ctx.save();
    ctx.translate(tempX(t), lumY(l));
    ctx.rotate(a * Math.PI / 180);
    ctx.font = `900 ${Math.max(17, Math.min(27, chart.width / 58))}px system-ui`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
...
function clickCanvas(e) {
  ...
  if (s) { state.selected = s; cardStar(s); draw(); return; }
  const r = regionAt(p);
  if (r) { state.selected = r; cardRegion(r); draw(); }
}
...
function focus(t, l, z = 2.3) {
  state.scale = clamp(z, 1, 8);
  state.tx = canvas.clientWidth / 2 - tempX(t) * state.scale;
  state.ty = canvas.clientHeight / 2 - lumY(l) * state.scale;
  updateZoom();
  draw();
}
...
function tempX(t) {
  const p = chart.plot;
  const a = Math.log10(chart.minTemp), b = Math.log10(chart.maxTemp);
...
function lumY(l) {
  const p = chart.plot;
  const a = Math.log10(chart.minLum), b = Math.log10(chart.maxLum);
