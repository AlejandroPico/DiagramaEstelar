...
(() => {
  ...
  ui.csv.onchange = importCatalogFiles;
  ui.status.textContent = 'Importador de catálogos preparado: NASA Exoplanet Archive, HYG/ATHYG, Gaia/Kaggle y CSV HR genéricos.';

  async function importCatalogFiles(event) {
    ...
    try {
      ...
      if (merged.length > 20000) {
        ...
        ui.toggles.animation.checked = false;
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      worker.onmessage = event => {
        ...
        if (msg.type === 'progress') {
          ui.status.textContent = total === 1 ? msg.message : `[${index}/${total}] ${msg.message}`;
        }
        if (msg.type === 'done') {
          worker.terminate();
  ...
  function mergeImportedStars(stars) {
    ...
    for (const star of stars) {
      ...
      if (!previous) map.set(key, star);
      else map.set(key, mergeStar(previous, star));
    }
...
function prepareCsv(text) {
  ...
  return {
    ...
    lines: lines.slice(headerIndex + 1).filter(line => !line.trim().startsWith('#'))
  };
}
...
function normalizeNasa(row, index) {
  ...
  return makeStar({
    ...
    className: classify(teff, luminosity),
    notes: planetCount ? 'Anfitriona de ' + planetCount + ' planeta(s) confirmado(s) en el archivo importado.' : 'Anfitriona de exoplanetas.',
    planetCount,
...
function normalizeHyg(row, index, label) {
  ...
  return makeStar({
    ...
    className: classify(teff, luminosity, row.spect),
...
function normalizeStellarClassification(row, index) {
  ...
  return makeStar({
    name: 'Estrella clase ' + (row.star_type || row.spectral_class || index + 1),
    teff,
    luminosity,
    radius: number(row.radius_r_ro),
    mass: NaN,
    distance_ly: NaN,
    designation: row.spectral_class || '',
    source: 'Stellar Classification / Kaggle',
    catalogKey: 'stellar-class:' + index,
    className: classFromStellarType(row.star_type, teff, luminosity),
...
function colorNameToHex(name) {
  ...
  const map = { red:'#ff6b4a', orange:'#ff9b5f', yellow:'#ffe89c', yellowwhite:'#fff4d4', white:'#f5f8ff', bluewhite:'#d8e9ff', blue:'#82adff', whitish:'#f5f8ff', pale:'#f5f8ff' };
  return map[key] || null;
}
